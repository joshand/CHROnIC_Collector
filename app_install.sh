...
#sed -i "" -e "s/DOCKERUSER/$docker_username/g" chronic_collector.json

echo " "
echo "***************************************************"
echo "Installing the app as  chroniccollector"
curl -k -X POST -u $mantl_user:$mantl_password https://$control_address:8080/v2/apps \
-H "Content-type: application/json" \
-d @chronic_collector.json \
| python -m json.tool
...
echo
echo "http://chroniccollector.$mantl_domain/"
echo
echo
